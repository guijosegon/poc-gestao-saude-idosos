...
namespace GestaoSaudeIdosos.Application.AppServices
{
    public class AppService<T> : IAppService<T> where T : class
    {
        ...
        public virtual IQueryable<T> AsQueryable(params Expression<Func<T, object?>>[] includes) => _service.AsQueryable(includes);
        public virtual IQueryable<T> AsTracking(params Expression<Func<T, object?>>[] includes) => _service.AsTracking(includes);
